...
public sealed class AnthropicResponseProcessor : IResponseProcessor
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogError(ex, "Error processing Anthropic streaming response for request {RequestId}", requestId);
        }
    }
        ...
        Func<BaseUsageResult, Task> onUsageDetected)
    {
        ...
        {
            _logger.LogWarning(ex, "Failed to parse Anthropic streaming event JSON: {JsonData}", jsonData);
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning(ex, "Failed to parse Anthropic response JSON for request {RequestId}", requestId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error processing Anthropic non-streaming response for request {RequestId}", requestId);
        }
    }
        ...
        Func<BaseUsageResult, Task> onUsageDetected)
    {
        ...
        {
            _logger.LogError(ex, "Error processing Anthropic usage data for request {RequestId}", requestId);
        }
    }
}