...
- **零拷贝流处理**: 使用 System.IO.Pipelines 实现高性能流处理
- **动态流检测**: 基于 Content-Type 响应头的智能响应类型检测
...
```
HTTP 请求 → 管道路由器 → [静态管道] → 静态文件/健康检查/提供商列表
                     ↓
               [网关管道] → 通用使用量中间件 → 路径模式中间件 → AI网关中间件 → AI 提供商
                                    ↓
                              响应处理器 → Token使用量流处理器 (Pipelines)
```

高级中间件管道，具备条件路由、实时使用量分析和高性能流处理。
...
- **AOT 编译**: 原生编译，启动更快，内存使用更少（<100ms 启动时间）
- **零拷贝流处理**: 使用 System.IO.Pipelines 实现内存高效的请求/响应处理
- **对象池化**: 通过智能对象重用减少 GC 压力（90%+ 分配减少）
- **高吞吐量**: OpenAI 15,000+ RPS，混合提供商 25,000+ RPS，亚毫秒级延迟
- **内存优化**: ~180-300MB 基准内存使用，支持连接池
- **动态流检测**: 基于响应 Content-Type 的自动检测和优化
